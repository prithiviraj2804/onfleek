...
{   
    if ($date != "" && $date != "0000-00-00") {
        ...
        if($CI->session->userdata('default_php_date_format')){
            $sessionFormat = $CI->session->userdata('default_php_date_format');
        }else{
            $sessionFormat = 'd/m/Y';
        }
        return $date->format($sessionFormat);
...
{
    $CI = &get_instance();
    $date = DateTime::createFromFormat($CI->session->userdata('default_php_date_format'), $date);
