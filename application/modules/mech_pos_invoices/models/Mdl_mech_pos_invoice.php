...
/**
 * Class Mdl_Mech_Invoice
 */
class Mdl_Mech_Pos_Invoice extends Response_Model
{
    ...
    public function default_select()
    {
        ...
        mech_invoice.visit_type,mech_invoice.mode_of_payment,mech_invoice.invoice_date,
        mech_invoice.invoice_date_created,mech_invoice.in_days,mech_invoice.invoice_date_due,
        mech_invoice.bank_id,mech_invoice.current_odometer_reading,mech_invoice.fuel_level,
        ...
        mech_invoice.is_credit,mech_invoice.cheque_no,mech_invoice.cheque_to,
        mech_invoice.bank_name,mech_invoice.online_payment_ref_no,mech_invoice.payment_method_id,
        mech_invoice.invoice_status,mech_invoice.invoice_category,mech_invoice.workshop_id,
        mech_invoice.parts_discountstate, mech_invoice.description,mech_invoice.service_discountstate,
        mech_invoice.description,mech_invoice.w_branch_id,
        mc.client_name,
    ...
    public function default_where(){
        ...
        $this->db->where('mech_invoice.invoice_category' , "P");
    ...
    public function validation_rules()
    {
        return array(
            ...
            `customer_car_id` => array(
                ...
                'label' => trans('lable422'),
                'rules' => 'required'
            ),
            `branch_id` => array(
                'field' => 'branch_id',
                'label' => trans('lable149'),
                'rules' => 'required'
            ),
            `shift` => array(
                'field' => 'shift',
	...
	public function reduceInvoicePayment($amount,$pos_invoice_id)
	{
		$this->db->set('total_paid_amount', 'total_paid_amount - '.$amount, FALSE);
		$this->db->set('total_due_amount', 'total_due_amount + '.$amount, FALSE);
		$this->db->where('invoice_id',$pos_invoice_id);
	...
	// public function get_user_quote_product_item($pos_invoice_id = null, $user_id  = null)
    // {
    //     if($pos_invoice_id){
    //     	$work_shop_id = $this->session->userdata('work_shop_id');
	// 		$branch_id = $this->session->userdata('branch_id');
    //         $this->db->select('si.item_id,si.invoice_id,si.quote_id,si.user_id,si.is_from,si.service_type,si.category_type,si.service_item,si.item_service_name,si.item_hsn,si.user_item_price,si.mech_item_price,si.item_qty,si.item_discount,si.item_amount,si.igst_pct,si.igst_amount,si.cgst_pct,si.cgst_amount,si.sgst_pct,si.sgst_amount,si.expiry_date,si.expiry_kilometer,si.warrentry_prd,si.item_total_amount,si.service_status,mp.product_id,mp.url_key,mp.product_name,mp.hsn_code,mp.sku_code,mp.product_category_id,mp.product_type,mp.unit_type,mp.reorder_quantity,mp.apply_for_all_bmv,mp.brand_id,mp.model_id,mp.variant_id,mp.kilo_from,mp.kilo_to,mp.mon_from,mp.mon_to,mp.fuel_type,mp.rack_no,mp.cost_price,mp.sale_price,mp.diff_amount,mp.tax_percentage,mp.description');
    ...
    //         $this->db->where('si.invoice_id', $pos_invoice_id);
    ...
    // }
    // public function get_user_quote_service_item($pos_invoice_id = null)
    // {
    //     if($pos_invoice_id){
    //     	$work_shop_id = $this->session->userdata('work_shop_id');
	// 		$branch_id = $this->session->userdata('branch_id');
    //         $this->db->select('si.item_id,si.invoice_id,si.quote_id,si.user_id,si.is_from,si.service_type,si.category_type,si.service_item,si.item_service_name,si.item_hsn,si.user_item_price,si.mech_item_price,si.item_qty,si.item_discount,si.item_amount,si.igst_pct,si.igst_amount,si.cgst_pct,si.cgst_amount,si.sgst_pct,si.sgst_amount,si.expiry_date,si.expiry_kilometer,si.warrentry_prd,si.item_total_amount,si.service_status,ci.sc_item_id,ci.service_item_name,ci.service_category_id,ci.is_popular,ci.service_item_short_description,ci.service_item_description');
    //         $this->db->from('mech_invoice_item si');
    //         $this->db->join('mechanic_service_category_items ci', 'ci.sc_item_id = si.service_item');
    //         if($this->session->userdata('user_type') == 3){
	// 			$this->db->where('si.workshop_id='.$work_shop_id.'');
	// 		}elseif($this->session->userdata('user_type') == 4){
	// 			$this->db->where('si.workshop_id='.$work_shop_id.' AND si.w_branch_id='.$branch_id.'');
