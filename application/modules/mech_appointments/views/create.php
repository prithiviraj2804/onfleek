...
function renderComments(){
	...
	$.post("<?php echo site_url('mech_appointments/ajax/get_comments'); ?>", {
		entity_id : entity_id,
	...
	function (data) {
		...
		if(list.success == 1){
			...
			for(var i=0; i<list.comments.length; i++){
				...
				htmll += '<span class="notesgray float_left ellipsistext cD" style="max-width: 100px;" title="Appointment">Appointment</span></span>';
...
function editComments(comment_id){
	...
	$.post("<?php echo site_url('mech_appointments/ajax/get_Comment_detials'); ?>", {
	...
	function (data) {
		...
		if(list.success == 1){
			...
			html += '</select></div></div>';
			html += '<div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-xs-12 text-right padding0px">';
			html += '<div class="col-lg-3 col-md-3 col-sm-3 col-xs-12 text-right">';
			...
			html += '<div class="col-lg-3 col-md-3 col-sm-3 col-xs-12 text-right paddingTop20px paddingLeft0px">';
			html += '<label class="control-label string required">Re-schedule Date</label></div>';
			html += '<div class="col-lg-9 col-md-9 col-sm-9 col-xs-12 paddingTop20px paddingLeft0px">';
...
function saveComments(comment_id, lead_id){
	...
	$.post("<?php echo site_url('mech_appointments/ajax/save_comments'); ?>", {
		ml_id : lead_id,
		comments: comments,
		comment_id: comment_id,
			...
			<div class="nav nav-tabs">
				<div class="tbl">
					<ul class="nav" role="tablist">
						...
						<?php if($mech_leads->ml_id && $mech_leads->lead_date){?>
						<li class="nav-item">
					...
					<div class="tab-content">
						<div role="tabpanel" class="tab-pane fade <?php echo $one_tab_active; ?>" id="tabs-2-tab-1">
							...
							<div class="form-group clearfix">
								...
								<div class="col-lg-9 col-md-9 col-sm-9 col-xs-12">
									...
									<div class="col-lg-12 paddingTop5px paddingLeft0px">
										<a href="javascript:void(0)" data-toggle="modal" data-model-from="leads" data-target="#addNewCar" class="add_client_page fontSize_85rem float_left">
											+ <?php _trans('lable48'); ?> 
										</a>
									</div>
								</div>
							...
							<div class="form-group clearfix">
								...
								<div class="col-lg-9 col-md-9 col-sm-9 col-xs-12">
									...
									<?php foreach($permission as $perlist){
										if($perlist->module_name == "customer"){ ?>
									<div class="col-lg-12 paddingTop5px paddingLeft0px">
							...
							<div class="form-group clearfix">
								<div class="col-sm-3 text-right">
									<label class="control-label string required"><?php _trans('lable527');?> *</label>
								</div>
								<div class="col-sm-9">
								...
								<input type="text" name="lead_date" id="lead_date" class="form-control reschedule_date removeError datepicker_error" value="<?php if($mech_leads->lead_date != "" && $mech_leads->lead_date != "0000-00-00 00:00:00"){echo date("d-m-Y H:i:s", strtotime($mech_leads->lead_date));}else { echo date('d-m-Y H:m:s'); }?>" autocomplete="off">								</div>
							</div>
							<div class="form-group clearfix">
								<div class="col-sm-3 text-right">
									<label class="control-label string required"><?php _trans('lable529');?></label>
								</div>
								<div class="col-sm-9">
							...
							<div class="form-group clearfix">
								<div class="col-sm-3 text-right">
									<label class="control-label string required"><?php _trans('lable530');?>*</label>
								</div>
								<div class="col-sm-9">
								<input type="hidden" id="existing_lead_status" value="<?php echo $mech_leads->lead_status;?>" >
									<select name="lead_status" id="lead_status" class="bootstrap-select bootstrap-select-arrow form-control removeError" data-live-search="true" autocomplete="off">
										...
										<?php foreach($mech_appointment_status as $mechAppointStatus){ ?>
											<option value="<?php echo $mechAppointStatus->mps_id; ?>" <?php if($mech_leads->lead_status == $mechAppointStatus->mps_id){ echo "selected";}?>><?php echo $mechAppointStatus->status_label;;?></option>
										<?php } ?>
									</select>
								</div>
							</div>
							<div class="form-group clearfix">
								<div class="col-sm-3 text-right">
									<label class="control-label string required"><?php _trans('lable536');?></label>
								</div>
								<div class="col-sm-9">
									<select name="pickup" id="pickup" class="bootstrap-select bootstrap-select-arrow form-control removeError" data-live-search="true" autocomplete="off">
										<option value=""><?php _trans('lable537');?></option>
										<option value="1" <?php if($mech_leads->pickup == 1){ echo "selected";}?>><?php _trans('lable522');?></option>
										<option value="2" <?php if($mech_leads->pickup == 2){ echo "selected";}?>><?php _trans('lable538');?></option>
									</select>
								</div>
							</div>
						...
						<div role="tabpanel" class="tab-pane fade <?php echo $two_tab_active; ?>" id="tabs-2-tab-2">
							...
							<div class="card-block invoice">
								<?php $this->layout->load_view('mech_appointments/partial_product_table'); ?>
							</div>
							<div class="error error_msg_product_name"></div>
							<?php } ?>
							<div class="card-block invoice">
								<?php $this->layout->load_view('mech_appointments/partial_service_table'); ?>
							</div>
							<div class="card-block invoice">
								<?php $this->layout->load_view('mech_appointments/partial_service_package_table'); ?>
							</div>
														...
														<td class="pL10 pR10 w100p pR">
															<div class="pB5 cBafter">
																...
																<div class="mT5 p5 f12 gray2 float_left cB lh20 notesBtmDet">
																	...
																	<span class="float_left pR5 notesgray dIB">
																	<?php if($commentslist->reschedule_date != "" && $commentslist->reschedule_date != "0000-00-00 00:00:00"){echo date("d-m-Y H:i:s", strtotime($commentslist->reschedule_date));}else { echo "";}?></span>
																	<?php } ?>
								...
								<div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-xs-12">
									...
									<div class="form-group clearfix">
										...
										<div class="form_controls col-lg-9 col-md-9 col-sm-9 col-xs-10 paddingTop10px text-left">
											<input type="checkbox" class="reschedule" id="reschedule" name="reschedule" value="N" autocomplete="off">
										</div>
									</div> 
									<div class="form-group clearfix" id="rescheduleBox" style="display:none;">
										<div class="col-lg-3 col-md-3 col-sm-3 col-xs-12 text-right paddingTop20px">
										...
										<div class="col-lg-9 col-md-9 col-sm-9 col-xs-12 paddingTop20px">
											<input type="text" name="reschedule_date" id="reschedule_date" class="form-control reschedule_date" value="" autocomplete="off">
										</div>
									</div>
...
$(document).ready(function(){
	...
	$(".btn_cancel").click(function () {
		window.location.href = "<?php echo site_url('mech_appointments'); ?>";
	});
	...
	$(".btn_submit").click(function () {
		...
		$.post('<?php echo site_url('mech_appointments/ajax/create'); ?>', {
			ml_id : $("#ml_id").val(),
			...
			existing_lead_status : $("#existing_lead_status").val(),
			...
			if(list.success=='1'){
				notie.alert(1, '<?php _trans('toaster1');?>', 2);
				setTimeout(function(){
					window.location = "<?php echo site_url('mech_appointments/form'); ?>/"+list.ml_id+"/2";
				}, 100);
	...
	$("#save_service_product").click(function () {
		...
		$.post('<?php echo site_url('mech_appointments/ajax/create_services_products'); ?>', {
			ml_id : $("#ml_id").val(),
			...
			discountstate: $("#discountstate").val()?$("#discountstate").val().replace(/,/g, ''):'',
			...
			description: $("#description").val()?$("#description").val().replace(/,/g, ''):'',
			action_from: 'A',
			_mm_csrf: $('input[name="_mm_csrf"]').val()
		}, function (data) {
			list = JSON.parse(data);
			if(list.success == '1'){
				notie.alert(1, '<?php _trans('toaster1');?>', 2);
				setTimeout(function(){
					window.location = "<?php echo site_url('mech_appointments/form'); ?>/"+list.ml_id+"/3";
				}, 100);
			}else{
				$('#gif').hide();
				notie.alert(3,'<?php _trans('toaster2');?>', 2);
