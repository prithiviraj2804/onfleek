...
class Ajax extends Admin_Controller
{
    ...
    public function create()
    {
        ...
        if ($this->mdl_mech_leads->run_validation('validation_rules')) {
            ...
            if(!empty($action_from)){
                $lead_detail = $this->mdl_mech_leads->where('ml_id', $ml_id)->get()->row();
			...
			if($this->input->post('existing_lead_status') != $this->input->post('lead_status')){
				if(!empty($lead_detail->device_token)){
					$notification_mobile_data = array(
						"body" => "Hi ".$lead_detail->client_name.", Your vehicle service appointment is at ".$lead_detail->reschedule_date.". Call 9841967890 with any questions. Thank you",
					...
					$target = array($lead_detail->device_token);
					send_notification($data, $target, $notification_mobile_data);
				}
			}
			

			if($this->session->userdata('appointment_E') == 1){
                if($lead_detail->client_email_id){
                    $this->load->helper('mailer');
                    $this->load->helper('mailer/phpmailer');
                    $message = $this->load->view('emails/appointment', array (
						'client_name' => $lead_detail->client_name,
						'client_number' => $lead_detail->client_contact_no,
						'client_email' => $lead_detail->client_email_id,
						'vehicle_number' => $lead_detail->car_reg_no,
						'brand_name' => $lead_detail->brand_name,
						'model_name' => $lead_detail->model_name,
						'full_address' => $lead_detail->full_address, 
						'zip_code' => $lead_detail->zip_code
                    ), true );
                    $subject = trans('label941');
                    if (mailer_configured()) {
                        $this->load->helper('mailer/phpmailer');
                        if (!email_notification($invoice_id = null, $pdf_template = null, $from, $lead_detail->client_email_id, $subject, $message, $cc = null, $bcc = null, $attachment_files = null)) {
                            $email_failed = true;
            ...
            if($this->session->userdata('appointment_P_S') == 1){
                if($lead_detail->client_contact_no){

					if($this->input->post('lead_status') == 5 || $this->input->post('lead_status') == 6){
						$txt = "We have received an appointment( ".$leads_no.") for your car ".$this->input->post('car_reg_no').". We will pick you up after confirmation.";
						$sms = send_sms(htmlspecialchars($lead_detail->client_contact_no),$txt);
						if($sms->status == "success"){
							$db_sms_array = array(
								'user_id' => $this->session->userdata('user_id'),
								'name' => $lead_detail->client_name,
								'email_id' => $lead_detail->client_email_id,
								'mobile_number' => $lead_detail->client_contact_no,
								'message' => $txt,
								'type' => 3,
								'status' => 'S',
					...
					if( $this->input->post('pickup') == 1 ){
						$txt = "Congrats!!! Your order is confirmed with us. Our car service expert will pick the car shortly..";
						$sms = send_sms(htmlspecialchars($lead_detail->client_contact_no),$txt);
						if($sms->status == "success"){
							$db_sms_array = array(
								'user_id' => $this->session->userdata('user_id'),
	...
	public function get_Comment_detials(){
		...
		$comments = $this->db->from('mech_comments')->where('mech_comments.comment_id',$comment_id)->where('mech_comments.entity_type','A')->where('mech_comments.status','A')->join('mech_employee', 'mech_employee.employee_id = mech_comments.modified_employee_id')->order_by("mech_comments.comment_id","desc")->get()->row();
	...
	public function get_comments(){
		...
		// $comments = $this->db->from('mech_comments')->where('mech_comments.entity_id',$entity_id)->where('mech_comments.entity_type','A')->where('mech_comments.status','A')->join('mech_employee', 'mech_employee.employee_id = mech_comments.modified_employee_id')->order_by("mech_comments.comment_id","desc")->get()->result();
		$comments = $this->db->select("mech_comments.comment_id,mech_comments.entity_id,mech_comments.comments,mech_comments.reschedule,mech_comments.reschedule_date,mech_comments.created_on,mech_employee.employee_name")->from('mech_comments')->where('mech_comments.entity_id',$entity_id)->where('mech_comments.entity_type','A')->where('mech_comments.status','A')->join('mech_employee', 'mech_employee.employee_id = mech_comments.modified_employee_id')->order_by("mech_comments.comment_id","desc")->get()->result();
		if(!empty($comments)){
			$response = array(
				'success' => 1,
				'comments' => $comments,
