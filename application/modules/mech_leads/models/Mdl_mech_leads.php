...
class Mdl_Mech_Leads extends Response_Model
{
    ...
    public function default_select()
    {
        ...
        mech_leads.description,mech_leads.parts_discountstate,mech_leads.service_discountstate,
        mech_leads.discountstate,
        ...
        mech_leads.total_taxable_amount,
        mech_leads.total_tax_amount,mech_leads.grand_total,
        mech_lead_status.status_label,mech_lead_source.lead_source_name,
	...
	public function default_join(){
		...
		$this->db->join('mech_lead_status', 'mech_lead_status.mlstatus_id = mech_leads.lead_status', 'left');
	...
	public function default_where(){
        $this->mdl_mech_leads->where('mech_leads.category_type', "L");
    ...
    public function validation_rules()
    {
        return array(
            ...
            'lead_date' => array(
                'field' => 'lead_date',
                'label' => trans('lable487'),
                'rules' => 'required'
            ),
            'lead_source' => array(
                'field' => 'lead_source',
                'label' => trans('lable499'),
			),
            'lead_status' => array(
                'field' => 'lead_status',
                'label' => trans('lable509'),
                'rules' => 'required'
            ),
    ...
    public function get_user_quote_service_item($ml_id = null, $user_id  = null)
    {
        if($ml_id){
        	$work_shop_id = $this->session->userdata('work_shop_id');
			$branch_id = $this->session->userdata('branch_id');
            $this->db->select('si.rs_item_id,si.is_from,si.service_item,si.item_hsn,si.user_item_price,si.mech_item_price,si.item_qty,si.item_discount,si.item_amount,si.igst_pct,si.igst_amount,si.item_total_amount,si.item_work_level,ci.msim_id,ci.service_item_name,ci.service_category_id');
            $this->db->from('mech_leads_items si');
