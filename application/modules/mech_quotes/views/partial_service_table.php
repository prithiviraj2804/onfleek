	...
	<div class="col-lg-12">
        ...
        <div class="col-lg-2 col-md-3 col-sm-4 col-xs-12" style="padding: 10px 15px;">
            ...
            <select id="service_discountstate" name="service_discountstate" onchange="service_discountstate()" class="col-lg-4 col-md-4 col-sm-4 col-xs-12 bootstrap-select bootstrap-select-arrow form-control">
                <option <?php if ($quotes_detail->service_discountstate == 1) { echo 'selected'; } ?> value="1">&#8377;</option>
                <option <?php if ($quotes_detail->service_discountstate == 2) { echo 'selected'; } ?> value="2">&#37;</option>
            </select>
        </div>
		<section class="box-typical" style="overflow-x: inherit;">
			...
			<?php
                ...
                <a class="float_left fontSize_85rem addservice" href="javascript:void(0)" data-toggle="modal" data-existing_service_ids="<?php echo $existing_service_ids; ?>" data-entity_type="Q" data-entity_id="<?php echo $quotes_detail->quote_id; ?>" data-model-from="quote" data-target="#addservice">
                    + <?php _trans('lable398'); ?>
                </a>
            </div>
				...
				<table class="display table table-bordered" id="service_item_table" style="table-layout: fixed;" >
					<thead>
						...
						<th width="10%;" style="width:10%;max-width:10%;" class="text-right">
							<span class="showservicerupee" <?php if($quotes_detail->service_discountstate != 2 ){ echo 'style="display:block"';}else{ echo 'style="display:none"';} ?> ><?php echo $this->mdl_settings->currencychanger($this->session->userdata('default_currency_code')); ?></span>
							<span class="showservicepercentage" <?php if($quotes_detail->service_discountstate == 2 ){ echo 'style="display:block"';}else{ echo 'style="display:none"';} ?>>%</span>&nbsp;<?php _trans('lable1207'); ?></th>
						...
						foreach (json_decode($service_list) as $service) {?>
							<tr  class="item" id="tr_<?php echo $service->item_id; ?>">
								...
								<td width="15%;" style="width:15%;max-width:15%;" class="text-center">
									...
									<input type="text" name="usr_lbr_price" class="usr_lbr_price form-control text-right" onblur="service_calculation(<?php echo $service->item_id; ?>)" value="<?php echo format_money(($service->user_item_price?$service->user_item_price:0),$this->session->userdata('default_currency_digit')); ?>">
								</td>
								<td width="10%;" style="width:10%;max-width:10%;" class="text-right">
									<input type="text" name="item_discount" class="item_discount form-control text-right" onblur="service_calculation(<?php echo $service->item_id; ?>)" value="<?php echo format_money(($service->item_discount?$service->item_discount:0),$this->session->userdata('default_currency_digit')); ?>">
								...
								<td width="15%;" style="width:15%;max-width:15%;" class="text-right">
									<input style="width:35%;float: left; border-bottom-right-radius: 0px;border-top-right-radius: 0px;" type="text" name="igst_pct" class="igst_pct form-control text-right" onkeyup="service_calculation(<?php echo $service->item_id; ?>)" value="<?php echo $service->igst_pct; ?>">
								...
								<td width="5%;" style="width:5%;max-width:5%;" class="text-center"><span onclick="remove_item(<?php echo $service->item_id; ?>,'service','mech_quotes','<?= $this->security->get_csrf_hash(); ?>');"><i class="fa fa-times"></i></span></td>
							</tr>
						<?php } } ?>
					</tbody>
