...
/**
 * Class Mech_Bank_List
 */
class Mech_Bank_List extends Admin_Controller
{
    /**
     * Mech_Bank_List constructor.
     */
    public function __construct()
    {
        parent::__construct();
        $this->load->model('mdl_mech_bank_list');
        $this->load->model('mech_expense/mdl_mech_expense');
        $this->load->model('payment_methods/mdl_payment_methods');
        $this->load->model('mech_employee/mdl_mech_employee');
        $this->load->model('mdl_workshop_bank_trans_dtls');
    }
    ...
    public function create($deposit_id = NULL)
    {
        ...
        if($deposit_id){
            ...
            $breadcrumb = "lable481";
            $uploadedImages = $this->mdl_workshop_bank_trans_dtls->getUploadedImages($transaction_details->url_key);
        }else{
        ...
        $this->layout->set(array(
            ...
            'url_key' => $this->mdl_workshop_bank_trans_dtls->get_url_key(),
            'bank_list' => $bank_list,
