...
function searchFilter(page_num) {
    ...
    $.post('<?php echo site_url('spare_invoices/ajax/get_filter_list'); ?>', {
        page : page_num,
        ...
        if(list.success=='1'){
            ...
            if(list.mech_invoices.length > 0){
                for(var v=0; v < list.mech_invoices.length; v++){
					...
					html += '</td>';
					html += '<td data-original-title="'+(list.mech_invoices[v].invoice_no?list.mech_invoices[v].invoice_no:"")+'" data-toggle="tooltip" class="textEllipsis"><a href="<?php echo site_url("spare_invoices/view/'+list.mech_invoices[v].invoice_id+'"); ?>">'+(list.mech_invoices[v].invoice_no?list.mech_invoices[v].invoice_no:"")+'</a></td>';
					...
					if(list.user_type == 3){
						if(list.mech_invoices[v].invoice_status == "FP"){
							...
							html += '<li><a href="<?php echo site_url("spare_invoices/create/'+list.mech_invoices[v].invoice_id+'");?>">';
					...
					html += '<li><a target="_blank" href="<?php echo site_url("spare_invoices/generate_pdf/'+list.mech_invoices[v].invoice_id+'"); ?>">';
					html += '<i class="fa fa-print fa-margin" aria-hidden="true"></i></i> <?php _trans("lable141"); ?></a></li>';
					if(list.user_type == 3){
						html += '<li><a href="javascript:void(0)" onclick="remove_entity(\''+list.mech_invoices[v].invoice_id+'\',\'spare_invoices\',\'spare_invoice\',\'<?= $this->security->get_csrf_hash() ?>\')">';
