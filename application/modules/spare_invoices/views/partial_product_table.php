	...
	<div class="col-lg-12">
        ...
        <div class="col-lg-2 col-md-3 col-sm-4 col-xs-12" style="padding: 10px 15px;">
            ...
            <select id="parts_discountstate" name="parts_discountstate" onchange="part_discountstate()" class="col-lg-4 col-md-4 col-sm-4 col-xs-12 bootstrap-select bootstrap-select-arrow form-control">
                <option <?php if ($invoice_detail->parts_discountstate == 1) { echo 'selected'; } ?> value="1">&#8377;</option>
                <option <?php if ($invoice_detail->parts_discountstate == 2) { echo 'selected'; } ?> value="2">&#37;</option>
            </select>
        </div>
        <section class="box-typical" style="overflow-x: inherit;">
            ...
            <?php
                ...
                <a class="float_left fontSize_85rem addproduct" href="javascript:void(0)" data-entity-type="SI" data-toggle="modal" data-existing_product_ids="<?php echo $existing_product_ids; ?>" data-entity-id="<?php echo $invoice_detail->invoice_id;?>" data-customer-id="<?php echo $invoice_detail->customer_id;?>" data-model-from="invoice" data-target="#addproduct">
                ...
                <table class="display table table-bordered" id="product_item_table" width="100%" style="width:100%;float:left;table-layout: fixed;">
                    <thead>
                        ...
                        <th width="7%;" style="width:7%;max-width:7%;" class="text-right">
                            <span class="showrupee" <?php if($invoice_detail->parts_discountstate != 2 ){ echo 'style="display:block"';}else{ echo 'style="display:none"';} ?> ><?php echo $this->mdl_settings->currencychanger($this->session->userdata('default_currency_code')); ?></span>
                            <span class="showpercentage" <?php if($invoice_detail->parts_discountstate == 2 ){ echo 'style="display:block"';}else{ echo 'style="display:none"';} ?>>%</span>&nbsp;
                    ...
                    <tbody>
                        ...
                        <tr class="item" id="tr_<?php echo $product->item_id; ?>">
                            <input type="hidden" name="item_id" class="item_id" value="<?php echo $product->item_id; ?>">
                            ...
                            <td width="40%;" style="width:40%;max-width:40%;padding-top:10px ! important;">
                                ...
                                <input type="hidden" name="item_product_id" class="item_product_id" id="item_product_id_<?php echo $product->item_id; ?>" value="<?php echo $product->service_item;?>">
                            ...
                            <td width="10%;" style="width:10%;max-width:10%;" class="text-right">
                                <input type="text" name="usr_lbr_price" class="usr_lbr_price form-control text-right" onblur="product_calculation(<?php echo $product->item_id; ?>)" value="<?php echo format_money(($product->user_item_price?$product->user_item_price:0),$this->session->userdata('default_currency_digit')); ?>">
                            ...
                            <td width="7%;" style="width:7%;max-width:7%;" class="text-right">
                                <input type="text" name="item_discount" class="item_discount form-control text-right" onblur="product_calculation(<?php echo $product->item_id; ?>)" value="<?php echo format_money(($product->item_discount?$product->item_discount:0),$this->session->userdata('default_currency_digit')); ?>">
                            ...
                            <td width="4%;" style="width:2%;max-width:2%;" class="text-center">
                                <span onclick="remove_item(<?php echo $product->item_id; ?>,'prod','spare_invoices','<?= $this->security->get_csrf_hash(); ?>');"><i class="fa fa-times"></i></span>
                            </td>
                        </tr>
                        <?php } } ?>
                    </tbody>
                    <tfoot class="product_total_calculations">
                        ...
                        <td class="text-center"><label class="total_product_qty">0</label></td>
                        <td class="text-right">
