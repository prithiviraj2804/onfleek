...
class Ajax extends Admin_Controller
{
    ...
    {
        parent::__construct();
        $this->load->model('email_bulk/mdl_email_bulk');
    ...
    public function create()
    {
        ...
        if(!empty($mailsubject) && !empty($mailbody) && count($client_list) > 0){
            $dbarray = array(
                ...
                'date' => date('Y-m-d'),
                'body' => $this->input->post('mailbody'),
                'attachment_url' => $attachment_url,
                'created_by' => $this->session->userdata('user_id'),
                'modified_by' => $this->session->userdata('user_id'),
                'created_on' => date('Y-m-d H:m:s'),
            );
            $this->db->insert('mech_email_notification_dtls' , $dbarray);
            ...
            if(!empty($insert_id) && count($client_list) > 0){
                ...
                if(count($dbclient) > 0){
                    $this->db->insert_batch('mech_clients_notification_email_dtls', $dbclient);
                    $resposne = array(
                        'status' => true,
                        'success' => 1
                    );
    ...
    public function get_filter_list(){
        ...
        if($this->input->post('from_date')){  
            $this->mdl_email_bulk->where('mech_email_notification_dtls.date >=',date_to_mysql($this->input->post('from_date')));
        }
        
        if($this->input->post('to_date')){
            $this->mdl_email_bulk->where('mech_email_notification_dtls.date <=',date_to_mysql($this->input->post('to_date')));
        }

        if($this->input->post('campaign_name')){
            $this->mdl_email_bulk->like('mech_email_notification_dtls.campaign_name',$this->input->post('campaign_name'));
        }

        if($this->input->post('email_status')){
            $this->mdl_email_bulk->where('mech_clients_notification_email_dtls.email_status', $this->input->post('email_status'));
        }
       
        $rowCount = $this->mdl_email_bulk->get()->result();
        ...
        $this->mdl_email_bulk->limit($limit,$start);
        $bulkemail = $this->mdl_email_bulk->get()->result();
        
        foreach($bulkemail as $key => $email){
            $bulkemail[$key]->pending_count = $this->mdl_email_bulk->get_email_status_count($email->id, 'P');
            
            $bulkemail[$key]->success_count = $this->mdl_email_bulk->get_email_status_count($email->id, 'S');
            
            $bulkemail[$key]->failed_count = $this->mdl_email_bulk->get_email_status_count($email->id, 'F');

            $bulkemail[$key]->cancelled_count = $this->mdl_email_bulk->get_email_status_count($email->id, 'C');
        }
