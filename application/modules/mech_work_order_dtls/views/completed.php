...
<div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12 text-center imageDynamicHeight" style="display: table;">
    <div class="tbl-cell">
        ...
        <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12 text-center">
            <a class="btn btn-sm btn-primary" href="<?php echo site_url('mech_work_order_dtls/book'); ?>">
                <i class="fa fa-plus"></i> <?php _trans('label929'); ?>
            </a>
        </div>
    </div>
</div>
...
<header class="page-content-header">
	<div class="container-fluid">
		<div class="tbl">
			<div class="tbl-row">
				<div class="tbl-cell">
					<h3><?php _trans('lable269'); ?> <small class="text-muted">( <?php echo count($work_orders); ?> )</small></h3>
				</div>
		...
		<div class="card-block">
            <div class="row col-xl-12 col-lg-12 col-md-12 col-sm-12 padding0px">
                ...
                <div class="form_group col-xl-4 col-lg-4 col-md-4 col-sm-4">
                    ...
                    <div class="form_controls">
                        <input onkeyup="searchFilter()" type="text" name="invoice_number" id="invoice_number" class="form-control" value="<?php echo $invoice_number;?>" autocomplete="off">
                    </div>
                </div>
                <div class="form_group col-xl-4 col-lg-4 col-md-4 col-sm-4">
                    ...
                    <div class="form_controls">
                        <select onchange="searchFilter()" name="customer_id" id="customer_id" class="searchSelect bootstrap-select bootstrap-select-arrow customerList " data-live-search="true">
                            ...
                            <?php foreach ($customer_list as $customer){
                                    if($customer_id == $customer->client_id){
                                        $selected = 'selected="selected"';
                                    }else{
                                        $selected = '';
                                    }
                                ?>
                                <option value="<?php echo $customer->client_id; ?>" <?php echo $selected; ?>><?php echo $customer->client_name." ".($customer->client_contact_no?"(".$customer->client_contact_no.")":"");  ?></option>
                            <?php } ?>
                        </select>
                    </div>
                </div>
                    ...
                    <table class="display table datatable table-bordered" cellspacing="0" width="100%">
                        ...
                        <?php if(count($work_orders)>0) {
                            ...
                            {    
                                ...
                                ?>
                            <tr>
                                <td class="text-center" data-original-title="<?php _htmlsc($work_ord_list->issue_date?date_from_mysql($work_ord_list->issue_date):"-"); ?>" data-toggle="tooltip" class="textEllipsis"><?php _htmlsc($work_ord_list->issue_date?date_from_mysql($work_ord_list->issue_date):"-"); ?></td>
                                ...
                                <td data-original-title="<?php _htmlsc($work_ord_list->brand_name."-".$work_ord_list->model_name."-".$work_ord_list->variant_name."-".$work_ord_list->car_model_year."-(".$work_ord_list->car_reg_no.")"); ?>" data-toggle="tooltip" class="textEllipsis"><?php _htmlsc($work_ord_list->brand_name."-".$work_ord_list->model_name."-".$work_ord_list->variant_name."-".$work_ord_list->car_model_year."-(".$work_ord_list->car_reg_no.")"); ?></td>
                                <td class="text-left" data-original-title="<?php if($work_ord_list->jobsheet_status == 'P'){
...
function searchFilter(page_num) {
    ...
    var jobsheet_no = $('#jobsheet_no').val()?$('#jobsheet_no').val():'';

    $.post('<?php echo site_url('mech_work_order_dtls/ajax/get_completed_filter_list'); ?>', {
        page : page_num,
        jobsheet_status : 'C',
        from_date : from_date,
        branch_id : branch_id,
        ...
        if(list.success=='1'){
            ...
            html += '<th class="text-center"><?php _trans("lable1018");?></th>';
            ...
            if(list.work_orders.length > 0){
                ...
                if(list.work_orders.length >= 4)
                {
                    ...
                    html += '<td data-original-title="'+(list.work_orders[v].jobsheet_no?list.work_orders[v].jobsheet_no:" ")+'" data-toggle="tooltip" class="textEllipsis"><a href="<?php echo site_url("mech_work_order_dtls/view/");?>'+list.work_orders[v].work_order_id+'/'+list.work_orders[v].jobsheet_status+'">'+(list.work_orders[v].jobsheet_no?list.work_orders[v].jobsheet_no:" ")+'</a></td>';
                    html += '<td data-original-title="'+(list.work_orders[v].client_name)+''+(list.work_orders[v].client_contact_no?" ( "+list.work_orders[v].client_contact_no+" ) ":"")+'" data-toggle="tooltip" class="textEllipsis">'+(list.work_orders[v].client_name)+''+(list.work_orders[v].client_contact_no?" ( "+list.work_orders[v].client_contact_no+" ) ":"")+'</td>';
                    html += '<td data-original-title="'+(list.work_orders[v].brand_name?list.work_orders[v].brand_name:" ")+''+(list.work_orders[v].model_name?list.work_orders[v].model_name:" ")+''+(list.work_orders[v].variant_name?list.work_orders[v].variant_name:" ")+''+(list.work_orders[v].car_model_year?list.work_orders[v].car_model_year:" ")+''+(list.work_orders[v].car_reg_no?list.work_orders[v].car_reg_no:" ")+'" data-toggle="tooltip" class="textEllipsis">'+(list.work_orders[v].brand_name?list.work_orders[v].brand_name:" ")+''+(list.work_orders[v].model_name?list.work_orders[v].model_name:" ")+''+(list.work_orders[v].variant_name?list.work_orders[v].variant_name:" ")+''+(list.work_orders[v].car_model_year?list.work_orders[v].car_model_year:" ")+''+(list.work_orders[v].car_reg_no?list.work_orders[v].car_reg_no:" ");
                    html += '<td class="text-left" data-original-title="';
                    ...
                    html += '<li><a href="javascript:void(0)" onclick="remove_jobsheet(\''+list.work_orders[v].work_order_id+'\',\'<?= $this->security->get_csrf_hash() ?>\')">';
