			...
			<div class="table-responsive">
				<table class="display table table-bordered" id="service_package_item_table" style="table-layout: fixed;" >
					<thead>
						...
						<th width="15%;" style="width:15%;max-width:15%;" class="text-right"><?php echo $this->mdl_settings->currencychanger($this->session->userdata('default_currency_code')); ?>&nbsp;<?php _trans('lable338'); ?></th>
					...
					if ($service_package_list) {
						...
						foreach (json_decode($service_package_list) as $service) {?>
							<tr  class="item" id="tr_<?php echo $service->rs_item_id; ?>">
								...
								<td width="15%" style="max-width:15%;width:15%;" class="text-left">
								...
								<td width="5%;" style="width:5%;max-width:5%;" class="text-center"><span onclick="remove_item(<?php echo $service->rs_item_id; ?>,'package','mech_work_order_dtls','<?= $this->security->get_csrf_hash(); ?>');"><i class="fa fa-times"></i></span></td>
							</tr>
						<?php } } ?>
