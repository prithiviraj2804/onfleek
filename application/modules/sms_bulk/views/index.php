...
<div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12 text-center imageDynamicHeight" style="display: table;">
    <div class="tbl-cell">
        ...
        <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12 text-center">
            <a class="btn btn-sm btn-primary" href="<?php echo site_url('sms_bulk/form'); ?>">
                <i class="fa fa-plus"></i> <?php _trans('lable1151'); ?>
            </a>
        </div>
    </div>
</div>
...
<header class="page-content-header">
    <div class="container-fluid">
        <div class="tbl">
            <div class="tbl-row">
                <div class="tbl-cell">
                    <h3><?php _trans('lable1160'); ?></h3>
                </div>
                <div class="tbl-cell pull-right">
                    <a class="btn btn-sm btn-primary" href="<?php echo site_url('sms_bulk/form'); ?>">
                        <i class="fa fa-plus"></i> <?php _trans('lable904'); ?>
                    </a>
                </div>
            </div>
        </div>
    ...
    <section class="card">
        ...
        <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 padding0px">
            ...
            <div class="form_group col-xl-4 col-lg-4 col-md-4 col-sm-4" style="display:none;">
                ...
                <div class="form_controls">
                    <select onchange="searchFilter()" name="sms_status" id="sms_status" class="searchSelect bootstrap-select bootstrap-select-arrow" data-live-search="true">
        ...
        <div id="posts_content">
            <div class="overflowScrollForTable">
                <table class="display table datatable table-bordered" cellspacing="0" width="100%">
                    ...
                    <tbody>
                        <?php if(count($bulksmslist)) { 
                            $i = 1;
                        foreach ($bulksmslist as $bulksms) { ?>
                        <tr>
                            <td class="text_align_center" data-original-title="<?php echo $i; ?>" data-toggle="tooltip" class="textEllipsis"><?php echo $i; ?></td>
                            <td class="text_align_center" data-original-title="<?php echo date_from_mysql($bulksms->date); ?>" data-toggle="tooltip" class="textEllipsis"><?php echo date_from_mysql($bulksms->date); ?></td>
                            <td data-original-title="<?php _htmlsc($bulksms->campaign_name); ?>" data-toggle="tooltip" class="textEllipsis"><?php _htmlsc($bulksms->campaign_name); ?></td>
                            <td class="text_align_center" data-original-title="<?php _htmlsc($bulksms->client_count);?>" data-toggle="tooltip" class="textEllipsis"><?php _htmlsc($bulksms->client_count); ?></td>
                            <td class="text_align_center" data-original-title="<?php echo $this->mdl_sms_bulk->get_sms_status_count($bulksms->id,'P'); ?>" data-toggle="tooltip" class="textEllipsis"><?php echo $this->mdl_sms_bulk->get_sms_status_count($bulksms->id,'P'); ?></td>
                            <td class="text_align_center" data-original-title="<?php echo $this->mdl_sms_bulk->get_sms_status_count($bulksms->id,'S'); ?>" data-toggle="tooltip" class="textEllipsis"><?php echo $this->mdl_sms_bulk->get_sms_status_count($bulksms->id,'S'); ?></td>
                            <td class="text_align_center" data-original-title="<?php echo $this->mdl_sms_bulk->get_sms_status_count($bulksms->id,'F'); ?>" data-toggle="tooltip" class="textEllipsis"><?php echo $this->mdl_sms_bulk->get_sms_status_count($bulksms->id,'F'); ?></td>
                            <td class="text_align_center" data-original-title="<?php echo $this->mdl_sms_bulk->get_sms_status_count($bulksms->id,'C'); ?>" data-toggle="tooltip" class="textEllipsis"><?php echo $this->mdl_sms_bulk->get_sms_status_count($bulksms->id,'C'); ?></td>
                        </tr>
...
function searchFilter(page_num) {
    ...
    var sms_status = $('#sms_status').val()?$('#sms_status').val():'';

    $.post('<?php echo site_url('sms_bulk/ajax/get_filter_list'); ?>', {
        page : page_num,
        from_date : from_date,
        to_date   : to_date,
        ...
        if(list.success=='1'){
            ...
            html += '</tr></thead><tbody>';
            if(list.bulksms.length > 0){
                for(var v=0; v < list.bulksms.length; v++){     
                    html += '<tr><td class="text_align_center" data-original-title="'+[v+1]+'" data-toggle="tooltip" class="textEllipsis">'+[v+1]+'</td>';
                    html += '<td class="text_align_center" data-original-title="'+(list.bulksms[v].date?formatDate(list.bulksms[v].date):"")+'" class="text_align_center" data-toggle="tooltip" class="textEllipsis">'+(list.bulksms[v].date?formatDate(list.bulksms[v].date):"")+'</td>';
                    html += '<td data-original-title="'+((list.bulksms[v].campaign_name)?list.bulksms[v].campaign_name:" ")+'" data-toggle="tooltip" class="textEllipsis">'+((list.bulksms[v].campaign_name)?list.bulksms[v].campaign_name:" ")+'</td>';
                    html += '<td class="text_align_center" data-original-title="'+list.bulksms[v].client_count+'" data-toggle="tooltip" class="textEllipsis">'+list.bulksms[v].client_count+'</td>';
                    html += '<td class="text_align_center" data-original-title="'+list.bulksms[v].pending_count+'" data-toggle="tooltip" class="textEllipsis">'+list.bulksms[v].pending_count+'</td>';
                    html += '<td class="text_align_center" data-original-title="'+list.bulksms[v].success_count+'" data-toggle="tooltip" class="textEllipsis">'+list.bulksms[v].success_count+'</td>';
                    html += '<td class="text_align_center" data-original-title="'+list.bulksms[v].failed_count+'" data-toggle="tooltip" class="textEllipsis">'+list.bulksms[v].failed_count+'</td>';
                    html += '<td class="text_align_center" data-original-title="'+list.bulksms[v].cancelled_count+'" data-toggle="tooltip" class="textEllipsis">'+list.bulksms[v].cancelled_count+'</td>';
                    html +='</tr>';
                } 
            }else{ 
