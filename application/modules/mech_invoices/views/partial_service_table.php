	...
	<div class="col-lg-12">
        ...
        <div class="col-lg-2 col-md-3 col-sm-4 col-xs-12" style="padding: 10px 15px;">
            ...
            <select id="service_discountstate" name="service_discountstate" onchange="service_discountstate()" class="col-lg-4 col-md-4 col-sm-4 col-xs-12 bootstrap-select bootstrap-select-arrow form-control">
                <option <?php if ($invoice_detail->service_discountstate == 1) { echo 'selected'; } ?> value="1">&#8377;</option>
                <option <?php if ($invoice_detail->service_discountstate == 2) { echo 'selected'; } ?> value="2">&#37;</option>
            </select>
        </div>
				...
				<table class="display table table-bordered" id="service_item_table" style="table-layout: fixed;" >
					<thead>
						...
						<th width="10%;" style="width:10%;max-width:10%;" class="text-right">
							<span class="showservicerupee" <?php if($invoice_detail->service_discountstate != 2 ){ echo 'style="display:block"';}else{ echo 'style="display:none"';} ?> ><?php echo $this->mdl_settings->currencychanger($this->session->userdata('default_currency_code')); ?></span>
							<span class="showservicepercentage" <?php if($invoice_detail->service_discountstate == 2 ){ echo 'style="display:block"';}else{ echo 'style="display:none"';} ?>>%</span>&nbsp;<?php _trans('lable1207'); ?></th>
					...
					<tbody>
						...
						foreach (json_decode($service_list) as $service) {?>
							<tr  class="item" id="tr_<?php echo $service->item_id; ?>">
								...
								<td width="5%;" style="width:5%;max-width:5%;" class="text-center"><span onclick="remove_item(<?php echo $service->item_id; ?>,'service','mech_invoices','<?= $this->security->get_csrf_hash(); ?>');"><i class="fa fa-times"></i></span></td>
							</tr>
						<?php } } ?>
					</tbody>
